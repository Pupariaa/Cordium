...
function link_matches(matches, replyObject) {
    ...
    for (const result of matches.slice(0, n)) {
        const date = new Date(result.createdTimestamp);
        const dateString = `${date.getUTCFullYear()}/${date.getUTCMonth()}/${date.getUTCDate()} ${date.getUTCHours()}:${date.getUTCMinutes()}:${date.getUTCSeconds()}`;
        replyObject.content += `\n- [${result.channel.name} > ${result.author.username} > ${dateString} > ${trimStr(result.content, 10)}](<https://discord.com/channels/${global.guild.id}/${result.channel.id}/${result.id}>)`;
    }
}

const reporters = [log_matches, link_matches, report_matches];

function generate_channelName_choices() {
    return Object.keys(global.channels.text).map(channelName => ({ name: channelName, value: JSON.stringify([channelName]) }));
}
...
async function search(regex, types = null, channelsName = null) {
    ...
    if (!channelsName) {
        channelsName = [...Object.keys(global.channels.text)];
    }

    const matches = [];
    const promises = channelsName.map(async channelName => {
        const channel = await global.Channel.get(channelName);
        const messages = await global.Channel.text.fetchAllMessages(channel);
...
module.exports = {
    ...
    .addStringOption(option =>
        option.setName('channel_name')
            ...
            .setRequired(false)
            .addChoices(generate_channelName_choices())
    )
    .addStringOption(option =>
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const channelsName = JSON.parse(interaction.options.getString('channel_name')) || [...Object.keys(global.channels.text)];
