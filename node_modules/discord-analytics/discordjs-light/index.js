...
class DiscordAnalytics {
    ...
    async calculateGuildMembersRepartition() {
        ...
        let guildsMembers = [];
        if (!this._sharded)
            guildsMembers = this._client.guilds.cache.map((guild) => guild.memberCount);
        else
            guildsMembers = [].concat(await this._client.shard?.broadcastEval((c) => c.guilds.cache.map((guild) => guild.memberCount)));
    ...
    async trackInteractions(interaction) {
        ...
        let guilds = [];
        this._client.guilds.cache.map((current) => guilds.find((x) => x.locale === current.preferredLocale) ?
        ...
        if (interaction.isCommand()) {
            this.statsData.interactions.find((x) => x.name === interaction.commandName && x.type === 2 /* InteractionType.APPLICATION_COMMAND */) ?
                ++this.statsData.interactions.find((x) => x.name === interaction.commandName && x.type === 2 /* InteractionType.APPLICATION_COMMAND */).number :
                this.statsData.interactions.push({ name: interaction.commandName, number: 1, type: 2 /* InteractionType.APPLICATION_COMMAND */ });
        }
        else if (interaction.isMessageComponent()) {
            this.statsData.interactions.find((x) => x.name === interaction.customId && x.type === 3 /* InteractionType.MESSAGE_COMPONENT */) ?
                ++this.statsData.interactions.find((x) => x.name === interaction.customId && x.type === 3 /* InteractionType.MESSAGE_COMPONENT */).number :
                this.statsData.interactions.push({ name: interaction.customId, number: 1, type: 3 /* InteractionType.MESSAGE_COMPONENT */ });
        }
        else if (interaction.isModalSubmit()) {
            this.statsData.interactions.find((x) => x.name === interaction.customId && x.type === 5 /* InteractionType.MODAL_SUBMIT */) ?
                ++this.statsData.interactions.find((x) => x.name === interaction.customId && x.type === 5 /* InteractionType.MODAL_SUBMIT */).number :
                this.statsData.interactions.push({ name: interaction.customId, number: 1, type: 5 /* InteractionType.MODAL_SUBMIT */ });
        }
        ...
        else if (member.permissions.has(8n) || member.permissions.has(32n))
            ++this.statsData.users_type.admin;
        else if (member.permissions.has(8192n) || member.permissions.has(2n) || member.permissions.has(4n) || member.permissions.has(4194304n) || member.permissions.has(8388608n) || member.permissions.has(16777216n) || member.permissions.has(1099511627776n))
            ++this.statsData.users_type.moderator;
        else if (member.joinedAt && member.joinedAt > oneWeekAgo)
  ...
  */
    trackEvents() {
        ...
        this._client.on("guildCreate", (guild) => this.trackGuilds(guild, "create"));
        this._client.on("guildDelete", (guild) => this.trackGuilds(guild, "delete"));
    }
}
