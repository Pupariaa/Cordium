...
Events.createReportEvent = function (filename) {
    return function (line, eventName, ...args) {
        if (!global.global.reportEvents || !global.configReportEvents[eventName]) return;
...
Events.createReportEventError = function (filename) {
    return function (line, eventName, err) {
        console.error(`${filename} - Line ${line} (${eventName}): `, err);
    }
};
