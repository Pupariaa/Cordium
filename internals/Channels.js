	...
	for (const [channelName, channelAlias] of Object.entries(global.configChannels[category])) {
		for (const tag of tags) {
			if (channelAlias.tags.includes(tag)) {
				const channel = _getByAlias(category, channelName);
...
class Channels {
	...
	getByAlias = function (channelName) {
		return this.text.getByAlias(channelName) || this.voice.getByAlias(channelName) || this.forum.getByAlias(channelName);
	}
