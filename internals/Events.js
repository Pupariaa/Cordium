...
async function dispatchEvents() {
	...
	dispatchEvent(Events.ChannelCreate, (channel) => channel.guild.id, async function (channel) {
	...
	dispatchEvent(Events.GuildBanAdd, (ban) => ban.guild.id, async function (ban) {
		const user = ban.user;
		const executor = this.latestAuditLog.executor;
	...
	dispatchEvent(Events.GuildMemberAdd, (member) => member.guild.id, async function (member) {
	...
	dispatchEvent(Events.GuildStickerCreate, (oldSticker, newSticker) => newSticker.guild.id, async function (oldSticker, newSticker) {
		const executor = this.latestAuditLog.executor;
	...
	dispatchEvent(Events.InteractionCreate, (interaction) => interaction.guildId, async function (interaction) {
	...
	dispatchEvent(Events.InviteCreate, (invite) => invite.guild.id, async function (invite) {
		const executor =
	...
	dispatchEvent(Events.MessageReactionAdd, (reaction, executor, details) => reaction.message.guild.id, async function (reaction, executor, details) {
	...
	dispatchEvent(Events.ThreadCreate, (thread, newlyCreated) => thread.guild.id, async function (thread, newlyCreated) {
		const executor = this.latestAuditLog.executor;
	...
	dispatchEvent(Events.ThreadUpdate, (oldThread, newThread) => newThread.guild.id, async function (oldThread, newThread) {
	...
	dispatchEvent(Events.VoiceStateUpdate, (oldState, newState) => newState.guild.id, async function (oldState, newState) {
